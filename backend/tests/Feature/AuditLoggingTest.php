...
class AuditLoggingTest extends TestCase
{
    ...
    {
        $actor = User::query()->create(['name'=>'Auditor','email'=>'auditor@example.test','password'=>'password']);
        $request = Request::create('/admin/settings', 'PATCH', server: ['REMOTE_ADDR'=>'192.0.2.10','HTTP_USER_AGENT'=>'FOODEX-Test']);

        $log = app(AuditLogger::class)->record('settings.updated', $actor, $actor, ['locale'=>'en'], ['locale'=>'ar'], $request);
    ...
    public function test_audit_payload_redacts_secrets_recursively(): void
    {
        $log = app(AuditLogger::class)->record('credentials.changed', null, null, null, [
            ...
            'profile'=>['api_key'=>'secret-key','display_name'=>'Safe Name'],
