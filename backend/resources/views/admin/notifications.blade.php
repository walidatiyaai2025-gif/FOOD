    ...
    <section class="panel">
        ...
        <form method="post" action="{{ route('admin.notifications.store') }}">@csrf
            <div class="grid">
                ...
                <div><label>{{ __('notifications.app') }}</label><select name="app">@foreach(['all','customer','driver','dashboard'] as $value)<option value="{{ $value }}">{{ __('notifications.app_options.'.$value) }}</option>@endforeach</select></div>
        ...
        <article class="card">
            ...
            <form method="post" action="{{ route('admin.notifications.update',$notification) }}">@csrf @method('PATCH')
                <div class="grid">
                    ...
                    <div><label>{{ __('notifications.app') }}</label><select name="app">@foreach(['all','customer','driver','dashboard'] as $value)<option value="{{ $value }}" @selected($notification->app===$value)>{{ __('notifications.app_options.'.$value) }}</option>@endforeach</select></div>
