    ...
    public function __invoke(Request $request): JsonResponse
    {
        ...
        if ($setting === null) {
            return response()->json([
                'message' => 'Mobile runtime settings are not configured.',
            ], 404);
        }

        $locale = $data['locale'] ?? 'ar';
        $push = PushProviderSetting::query()
            ->where('app', $setting->app)
            ->where('environment', $setting->environment)
            ->get()
            ->mapWithKeys(fn (PushProviderSetting $provider): array => [
                $provider->platform => [
                    'enabled' => (bool) $provider->enabled,
                    'provider' => $provider->provider,
                    'default_sound' => $provider->default_sound,
                    'default_channel' => $provider->default_channel,
                    'default_icon' => $provider->default_icon,
                    'default_category' => $provider->default_category,
                ],
            ]);
        ...
        return response()->json([
            'data' => [
                ...
                'deep_links' => $setting->deep_link_config ?? [],
                'store_readiness' => $setting->store_readiness ?? [],
                'push' => $push,
            ],
        ]);
    }
